    ...
    <div id="sb-site">
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="example-box-wrapper">
                                        ...
                                        <table class="table table-bordered table-hover" id="tab_logic">
                                            ...
                                            <tbody id="tbody_">
                                                <tr id='addr0'>
                                                    <td colspan="6" class="text-center">Seleccione un rango de fechas</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-horizontal bordered-row">
                                    ...
                                    <div class="form-group">
                                        <label class="col-sm-3 control-label">Por tipo de transacción</label>
                                        <div class="col-sm-9">
        ...
        $(function() {
            ...
            var tablaDatos = function(btn){
                $.ajax({
                    url: '<?= base_url() ?>devolverventa/listarreporte',
                    type: 'POST',
                    data: {
                        'fechaf': fechaf,
                        'fechai':fechai,
                    ...
                    beforeSend: function () {
                        ...
                        $('#tbody_').html('<tr> <td colspan="6">Buscando datos <i class="fa fa-refresh fa-spin"></i></td>');
                    }
                }).done(function (data) {
                    btn.html('Aplicar');
                    ...
                    for(var i in data){
                        ...
                        html +='<td class="">'+(d.motivo)+'</td>';
                        html +='<td class="">'+(d.resultado)+'</td>';
                        html +='<td class="">'+(d.tipo=='c'?'Compra':'Venta').toUpperCase()+'</td>';
                        html +='</tr>';
                        $("#tbody_").append(html);
                        
                    }
                    if(data.length < 1){
                        $('#tbody_').html('<tr> <td colspan="6">Ningún dato en la tabla</td>');
                    }
                    $('#modal_buscar').modal('hide');
                }).fail(function () {
                    $('#tbody_').html('<tr> <td colspan="6">Error,  intente nuevamente</td>');
                });
            }
            ...
            $('#exportar').click(function(){
                window.location= '<?= base_url() ?>devolverventa/pdf?tipo='+$('#tipo').val()+'&fechai='+fechai+'&fechaf='+fechaf;
            });
